...
import { NegamaxPlayer, Move } from './gomoku-ai-typescript';
...
class SimpleGomokuTest {
    ...
    printBoard(): void {
        ...
        // Column numbers
        console.log('   ', Array.from({ length: this.boardSize }, (_, i) =>
    ...
    async getAIMove(): Promise<Move | null> {
        console.log('\n🤔 AI is thinking...');
    ...
    async runTest1_OpeningMoves() {
        ...
        if (aiMove) {
            ...
            const distance = Math.max(
                Math.abs(aiMove.row - 7),
                Math.abs(aiMove.col - 7)
            );
