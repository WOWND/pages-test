...
class Field {
    readonly row: number;
    readonly col: number;
    index: number; // 0: empty, 1: player1, 2: player2, 3: out of bounds

    constructor(row: number = 0, col: number = 0, outOfBounds: boolean = false) {
        this.row = row;
        this.col = col;
...
class State {
    ...
    terminal(): number {
        if (this.moveStack.length === 0) return 0;
    ...
    private generateDirections(): void {
...
class NegamaxPlayer {
    ...
    private timeLimit: number; // in milliseconds
    ...
    private getSortedMoves(state: State): Move[] {
        ...
        if (fourMoves.length > 0) return fourMoves;
        if (opponentFourMoves.length > 0) return opponentFourMoves;
...
// For Firebase integration, you might want to add:
export interface GomokuAI {
    getMove(moves: Move[], boardSize: number): Move | null;
    cancelSearch(): void;
}

export function createAI(boardSize: number = 15): GomokuAI {
    const player = new NegamaxPlayer(boardSize);
    return {
        getMove: (moves: Move[], size: number) => player.getMove(moves, size),
